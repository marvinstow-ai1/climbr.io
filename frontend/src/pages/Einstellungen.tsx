...
export default function Einstellungen() {
  ...
  if (loadError) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
        ...
        <p className="mt-4 text-red-600" role="alert">{loadError}</p>
      </div>
    );
  }
  ...
  if (!data) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
        <h1 className="text-2xl font-bold">Einstellungen</h1>
        <p className="mt-4 text-slate2">Lade…</p>
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-3xl space-y-8 px-4 py-8 sm:px-6" data-testid="settings-page">
      ...
      <Section title="Profil">
        ...
        <form onSubmit={changePassword} className="mt-4 space-y-3">
          ...
          <div>
            <label htmlFor="new-pw" className="block text-xs text-slate2">Neues Passwort</label>
            <input
              id="new-pw"
              type="password"
              ...
              required
            />
            <p className={`mt-1 text-xs ${pw.length >= 8 ? "text-green-600" : "text-slate2"}`}>
              {pw.length >= 8 ? "✓ Mindestens 8 Zeichen" : "Mindestens 8 Zeichen"}
            </p>
          </div>
          <div>
            <label htmlFor="confirm-pw" className="block text-xs text-slate2">Bestätigen</label>
            <input
              id="confirm-pw"
              type="password"
              autoComplete="new-password"
      ...
      <Section title="Mein Plan">
        <Row label="Aktueller Plan">
          <span
            ...
            className="inline-flex items-center rounded-md bg-primary-50 px-2 py-0.5 text-sm font-medium text-primary"
          >
        ...
        <p className="mt-4 rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate2">
          Mehr Keywords, mehr Audits, mehr Kontrolle —{" "}
          <button
            type="button"
            disabled
            className="font-medium text-primary opacity-50"
            title="Kommt bald"
          >
            jetzt upgraden
          </button>{" "}
      ...
      <Section title="Benachrichtigungen">
        ...
        <div className="mt-4 opacity-60">
          <Toggle
            ...
            onChange={() => undefined}
          />
          <p className="ml-9 mt-1 text-xs text-slate2">Kommt bald.</p>
        </div>
      </Section>
    </div>
  );
  ...
  return (
    <label htmlFor={id} className="flex cursor-pointer items-start gap-3">
      ...
      <span>
        ...
        {description && <span className="block text-xs text-slate2">{description}</span>}
      </span>
    </label>
  );
}
