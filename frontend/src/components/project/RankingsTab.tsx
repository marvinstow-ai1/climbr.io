...
export default function RankingsTab({ projectId, token, gscConnected, onNotify }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      {planLimit && (
        <div className="rounded-lg border border-amber-500/20 bg-amber-500/10 p-4 text-sm" role="alert">
          <p className="font-medium text-amber-300">{planLimit.message}</p>
      ...
      <form onSubmit={onAdd} className="card flex flex-col gap-3 sm:flex-row sm:items-end">
        <div className="flex-1">
          ...
          />
          <p id="kw-help" className="mt-1 text-xs text-ink-muted">
      ...
      {loading ? (
        <p className="text-ink-muted">Lade Rankings…</p>
      ) : rows.length === 0 ? (
        <p className="text-ink-muted">Noch keine Keywords getrackt. Füg oben eines hinzu.</p>
      ) : (
        <ul className="divide-y divide-line rounded-xl border border-line bg-white/[0.03] backdrop-blur-md">
          {rows.map((row) => {
