...
export function ExplainerBox({
  ...
  return (
    <div
      className="rounded-lg border border-line bg-white/[0.03] backdrop-blur-md"
    ...
    >
      ...
      >
        <span className="flex items-center gap-2 text-sm font-medium text-ink">
          <span
            aria-hidden="true"
            className="flex h-5 w-5 items-center justify-center rounded-full bg-accent-dim text-xs font-bold text-accent"
          >
            ?
          </span>
          {title}
      ...
      {open && (
        <div
          ...
          className="space-y-3 border-t border-line px-4 py-3 text-sm text-ink-muted"
        >
          <p>{explanation}</p>
          {whyItMatters && (
            <p>
