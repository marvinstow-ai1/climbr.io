...
export function WikiBody({ body }: Props) {
  ...
  return (
    <div className="space-y-4 text-sm leading-relaxed text-slate2">
  ...
  switch (b.kind) {
    ...
    case "pre":
      return (
        <pre
          key={key}
          className="overflow-x-auto rounded-md bg-slate-900 p-3 text-xs leading-snug text-slate-100"
        >
          <code>{b.code}</code>
        </pre>
      );
          ...
          <table className="w-full border-collapse text-xs">
            <thead className="bg-slate-50">
              <tr>
                {b.head.map((h, j) => (
                  <th
                    key={j}
                    className="border border-slate-200 px-2 py-1 text-left font-semibold text-ink"
                  >
                    {renderInline(h)}
                  </th>
                ))}
            ...
            <tbody>
              {b.rows.map((row, j) => (
                <tr key={j}>
                  {row.map((cell, k) => (
                    <td key={k} className="border border-slate-100 px-2 py-1">
                      {renderInline(cell)}
                    </td>
                  ))}
                </tr>
...
function renderInline(text: string): ReactNode {
  ...
  return tokens.map((t, i) => {
    if (t.kind === "code")
      return (
        <code key={i} className="rounded bg-slate-100 px-1 py-0.5 font-mono text-xs">
          {t.text}
        </code>
      );
    ...
    if (t.kind === "link")
      return (
        <a key={i} href={t.url} className="text-primary underline" target="_blank" rel="noopener noreferrer">
          {t.text}
        </a>
      );
